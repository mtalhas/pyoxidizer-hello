...
def make_exe(dist):
    ...
    #exe.add_in_memory_extension_module(PythonExtensionModule="padding")
    exe.add_in_memory_python_resources(dist.pip_install(["jwcrypto"]))
...
def make_embedded_resources(exe):
    return exe.to_embedded_resources("_padding.abi3.so")
